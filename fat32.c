...
static uint32_t fat32_read_cluster_chain(uint32_t cluster, uint8_t* buffer, uint32_t buffer_size);
static fs_node_t* fat32_find_directory_entry(fs_node_t* parent, const char* name);
...
fs_node_t* fat32_mount(uint16_t bus, uint8_t drive, uint32_t partition_lba_start) {
    ...
    if (ata_read_sector(ata_bus, ata_drive, partition_lba_start, boot_sector_buffer) != 0) {
    ...
    cluster_begin_lba = fat_begin_lba + (boot_sector.BPB_NumFATs * boot_sector.BPB_FATSz32);
...
int fat32_format(int* progress) {
    ...
    *progress = 15;
    ...
    uint32_t total_clusters = (total_sectors - bpb->BPB_RsvdSecCnt) / bpb->BPB_SecPerClus;
    ...
    *progress = 40;
    ...
    ata_write_sector(bus, drive, bpb->BPB_RsvdSecCnt + fat_sz32, (uint16_t*)fat_table);

    memset(aligned_buffer, 0, sector_size);
    for (uint32_t i = 1; i < fat_sz32; i++) {
        ata_write_sector(bus, drive, bpb->BPB_RsvdSecCnt + i, aligned_buffer);
        ata_write_sector(bus, drive, bpb->BPB_RsvdSecCnt + fat_sz32 + i, aligned_buffer);
        if ((i % 8) == 0) {
    ...
    *progress = 80;
    ...
    uint32_t data_area_start = bpb->BPB_RsvdSecCnt + (bpb->BPB_NumFATs * fat_sz32);
    uint32_t root_dir_lba = data_area_start + ((bpb->BPB_RootClus - 2) * bpb->BPB_SecPerClus);
...
static uint32_t fat32_read(fs_node_t* node, uint32_t offset, uint32_t size, uint8_t* buffer) {
    ...
    while (bytes_to_read > 0 && current_cluster < 0x0FFFFFF8) {
        ...
        uint32_t cluster_lba = cluster_begin_lba + (current_cluster - 2) * boot_sector.BPB_SecPerClus;
        
        for (int i = 0; i < boot_sector.BPB_SecPerClus; i++) {
             ata_read_sector(ata_bus, ata_drive, cluster_lba + i, (uint16_t*)(cluster_buffer + i * boot_sector.BPB_BytsPerSec));
        }
