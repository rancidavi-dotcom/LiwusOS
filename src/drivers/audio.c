...
/* Which hardware backend is driving playback. */
enum {
    AUDIO_BACKEND_NONE = 0,
    AUDIO_BACKEND_AC97,
    AUDIO_BACKEND_HDA
};
static int audio_backend = AUDIO_BACKEND_NONE;
...
/* Fills `frames` interleaved stereo/16-bit frames into `dst`, padding the
 * tail with silence. Returns the number of real (non-silence) frames. */
static uint32_t fill_frames(audio_src_t *s, int16_t *dst, uint32_t frames) {
    uint32_t got = 0;

    if (s->type == AUDIO_SRC_STREAM) {
        if (s->stream_stop && *s->stream_stop)
            return 0;
        got = s->stream_fill(s->stream_user, dst, frames);
        if (s->stream_stop && *s->stream_stop)
            got = 0;
        if (got > frames)
            got = frames;
        for (uint32_t f = got; f < frames; f++) {
            dst[f * 2 + 0] = 0;
            dst[f * 2 + 1] = 0;
        }
        if (s->volume != 100) {
...
/* Fills the AC'97 ring chunk `idx` (interleaved stereo/16-bit). */
static uint32_t fill_chunk(audio_src_t *s, uint32_t idx) {
    return fill_frames(s, audio_chunks[idx], AC97_CHUNK_FRAMES);
}

/* Producer callback handed to the HDA backend: fills arbitrary buffers. */
static uint32_t hda_fill_shim(void *user, int16_t *dst, uint32_t max_frames) {
    return fill_frames((audio_src_t *)user, dst, max_frames);
}
...
static int ac97_playback_run(audio_src_t *s) {
...
/* Dispatch playback to the active hardware backend. */
static int playback_run(audio_src_t *s) {
    if (audio_backend == AUDIO_BACKEND_HDA)
        return hda_play(hda_fill_shim, s, s->rate, (int)s->volume,
                        &audio_busy);
    return ac97_playback_run(s);
}
...
static int audio_init_ac97(void) {
    pci_device_t *dev = pci_get_audio();
    if (!dev) {
        serial_print("AC97: no audio controller found\n");
        return -1;
    }
...
int audio_init(void) {
    ...
    /* Prefer the legacy AC'97 controller when present, then fall back to
     * Intel HD Audio.  Only one backend drives the shared playback API. */
    if (audio_init_ac97() == 0) {
        audio_backend = AUDIO_BACKEND_AC97;
        audio_present = 1;
        return 0;
    }

    if (hda_init() == 0) {
        audio_backend = AUDIO_BACKEND_HDA;
        audio_present = 1;
        audio_rate = hda_get_rate();
        audio_volume = hda_get_volume();
        return 0;
    }

    serial_print("audio: nenhum controlador (usando PC speaker)\n");
    return -1;
}
