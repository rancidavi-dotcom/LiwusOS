...
extern void xhci_init(pci_device_t *dev);
extern void usb_poll_xhci(void);
...
static void usb_poll_uhci(void) {
    ...
    for (int s = 0; s < count; s++) {
        ...
        // Find a registered UHCI device matching the type
        usb_device_t *udev = usb_devices;
        while (udev && !(udev->hcd == 1 && udev->type == dev_type)) udev = udev->next;
        if (udev) {
...
void usb_init() {
    ...
    // Procura por controladores xHCI (USB 3.0)
    pci_device_t *xhci = pci_get_usb(0x30);
    if (xhci) {
        serial_print("USB: Found xHCI Controller at PCI\n");
        xhci_init(xhci);
    }
