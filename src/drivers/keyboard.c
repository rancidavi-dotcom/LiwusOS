...
static keyboard_layout_t current_layout = KB_LAYOUT_ABNT2;
...
static unsigned char kbd_us_intl[128] = {
    [0x01] = 27,   [0x02] = '1',  [0x03] = '2',  [0x04] = '3',  [0x05] = '4',
    [0x06] = '5',  [0x07] = '6',  [0x08] = '7',  [0x09] = '8',  [0x0A] = '9',
    [0x0B] = '0',  [0x0C] = '-',  [0x0D] = '=',  [0x0E] = '\b', [0x0F] = '\t',
    [0x10] = 'q',  [0x11] = 'w',  [0x12] = 'e',  [0x13] = 'r',  [0x14] = 't',
...
static unsigned char kbd_us_intl_shift[128] = {
    [0x01] = 27,   [0x02] = '!',  [0x03] = '@',  [0x04] = '#',  [0x05] = '$',
    [0x06] = '%',  [0x07] = '^',  [0x08] = '&',  [0x09] = '*',  [0x0A] = '(',
    [0x0B] = ')',  [0x0C] = '_',  [0x0D] = '+',  [0x0E] = '\b', [0x0F] = '\t',
    [0x10] = 'Q',  [0x11] = 'W',  [0x12] = 'E',  [0x13] = 'R',  [0x14] = 'T',
...
static unsigned char kbd_us_intl_altgr[128] = {
    [0x12] = 0xE9,
    [0x16] = 0xFA,
    [0x17] = 0xED,
    [0x18] = 0xF3,
...
static const unsigned char *get_base_table(void) {
    switch (current_layout) {
        case KB_LAYOUT_US: return kbd_us;
        case KB_LAYOUT_US_INTL: return kbd_us_intl;
        default: return kbd_abnt2;
    }
}

static const unsigned char *get_shift_table(void) {
    switch (current_layout) {
        case KB_LAYOUT_US: return kbd_us_shift;
        case KB_LAYOUT_US_INTL: return kbd_us_intl_shift;
        default: return kbd_abnt2_shift;
    }
}

static const unsigned char *get_altgr_table(void) {
    switch (current_layout) {
        case KB_LAYOUT_US_INTL: return kbd_us_intl_altgr;
        default: return NULL;
    }
}
...
void keyboard_handler(void) {
    ...
    if (scancode & 0x80) {
        ...
        if (extended) {
            char key = 0;
            if (scancode == 0x48) key = 128;
            else if (scancode == 0x50) key = 129;
            else if (scancode == 0x4B) key = 130;
            else if (scancode == 0x4D) key = 131;
        ...
        if (scancode == 0x1D) {
            ...
            if (altgr_pressed && current_layout == KB_LAYOUT_US_INTL) {
                const unsigned char *altgr = get_altgr_table();
                if (altgr && scancode < 128 && altgr[scancode]) {
                    key = altgr[scancode];
                }
            }
            ...
            if (!key) {
                if (scancode == 0x73) key = shift_pressed ? '?' : '/';
                    ...
                    const unsigned char *base_tbl = shift_pressed ? get_shift_table() : get_base_table();
                    unsigned char base = base_tbl[scancode];
                    if (base >= 'a' && base <= 'z') {
                        key = base - 'a' + 1;
...
bool check_alt_f4(void) { return false; }
bool check_win_key(void) { return false; }
...
const char *keyboard_layout_name(keyboard_layout_t layout) {
    switch (layout) {
        ...
        case KB_LAYOUT_US_INTL: return "US International";
        default: return "ABNT2";
    }
}