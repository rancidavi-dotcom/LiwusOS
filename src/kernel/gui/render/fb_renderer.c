...
static void fb_present(gui_renderer_t *r) {
    ...
    /* Cópia em palavras de 32 bits (sem SSE — fast_memcpy corrompia XMM
     * entre tasks). byte-a-byte era muito lento no QEMU/TCG. */
    uint32_t *dst = (uint32_t *)s->vram;
    const uint32_t *src = (const uint32_t *)s->backbuf;
    uint32_t n = bytes / 4;
    while (n--) *dst++ = *src++;
}
