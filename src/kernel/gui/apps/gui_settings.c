...
static void show_system_settings(node_t *btn, void *userdata) {
    ...
    // Header
    node_t *title = label_create("sys_title", 0, 0, "System Information", 0xFFE6E8EB);
    title->margin[2] = 20; // Bottom margin
    node_add_child(s_content_panel, title);
    
    node_add_child(s_content_panel, label_create("sys_os", 0, 0, "OS: LiwusOS x86_64", 0xFFE6E8EB));
    node_add_child(s_content_panel, label_create("sys_ver", 0, 0, "Version: 1.0.0 (Pre-Alpha)", 0xFFE6E8EB));
    ...
    node_add_child(s_content_panel, label_create("sys_cpu", 0, 0, cpu_brand, 0xFF99A1AF));
    ...
    node_add_child(s_content_panel, label_create("sys_mem", 0, 0, mem_str, 0xFF99A1AF));
    ...
    node_add_child(s_content_panel, label_create("sys_disk", 0, 0, disk_str, 0xFF99A1AF));
    ...
    node_add_child(s_content_panel, label_create("sys_disp", 0, 0, disp_str, 0xFF99A1AF));
    
    // Uptime
    char up_str[64] = "Uptime: ";
    uint32_t up_secs = timer_ticks / 100;
    itoa(up_secs, buf, 10); strcat(up_str, buf); strcat(up_str, " seconds");
    node_add_child(s_content_panel, label_create("sys_up", 0, 0, up_str, 0xFF99A1AF));
    
    layout_engine_compute(s_settings_win);
}

/* ------------------------------------------------------------------ */
/* Chicago95 controls (checkboxes / radios)                            */
/* ------------------------------------------------------------------ */

static void toggle_scanlines(node_t *control, void *userdata, bool checked) {
    (void)control; (void)userdata;
    compositor_set_scanlines(checked);
}

static void scheme_radio_toggled(node_t *control, void *userdata, bool checked) {
    if (!checked) return;
    /* Keep the radio group exclusive: uncheck siblings under same parent. */
    if (control->parent) {
        for (uint32_t i = 0; i < control->parent->child_count; i++) {
            node_t *sib = control->parent->children[i];
            if (sib && sib != control && sib->type == NODE_RADIO) {
                checkbutton_set_checked(sib, false);
            }
        }
    }
    uint32_t color = (uint32_t)(uint64_t)userdata;
    theme_engine_set_color(THEME_COLOR_BACKGROUND, color);
}
...
static void show_display_settings(node_t *btn, void *userdata) {
    ...
    node_t *title = label_create("disp_title", 0, 0, "Display Settings", 0xFFE6E8EB);
    title->margin[2] = 20;
    node_add_child(s_content_panel, title);
    ...
    if (edid_get_monitor_info(&edid)) {
        ...
        node_add_child(s_content_panel, label_create("disp_mon", 0, 0, mon_str, 0xFFE6E8EB));
        
        // Year
        char year_str[64] = "Manufactured: Year ";
        itoa(edid.year_of_manufacture, buf, 10);
        strcat(year_str, buf);
        node_add_child(s_content_panel, label_create("disp_year", 0, 0, year_str, 0xFF99A1AF));
        ...
        node_add_child(s_content_panel, label_create("disp_max", 0, 0, max_str, 0xFF99A1AF));
        ...
        node_add_child(s_content_panel, label_create("disp_min", 0, 0, min_str, 0xFF99A1AF));
        
        // Refresh Rate
        char hz_str[64] = "Refresh Rate: ";
        itoa(edid.refresh_rate_hz, buf, 10); strcat(hz_str, buf); strcat(hz_str, " Hz");
        node_add_child(s_content_panel, label_create("disp_hz", 0, 0, hz_str, 0xFF99A1AF));
    } else {
        node_add_child(s_content_panel, label_create("disp_err", 0, 0, "EDID: Monitor Detection Failed", 0xFFFF5F57));
    }
    ...
    // Chicago95 controls ----------------------------------------------------
    node_t *cb_scan = checkbox_create("cb_scanlines", 0, 0, "Enable scanlines effect",
                                      compositor_get_scanlines());
    cb_scan->margin[0] = 20;
    checkbutton_set_on_toggle(cb_scan, toggle_scanlines, NULL);
    node_add_child(s_content_panel, cb_scan);

    node_add_child(s_content_panel, label_create("disp_scheme", 0, 0, "Desktop Color Scheme:", 0xFF99A1AF));

    node_t *rb_classic = radio_create("rb_classic", 0, 0, "Classic Teal",
                                      theme_engine_get_color(THEME_COLOR_BACKGROUND) == 0xFF008080);
    checkbutton_set_on_toggle(rb_classic, scheme_radio_toggled, (void *)(uint64_t)0xFF008080);
    node_add_child(s_content_panel, rb_classic);

    node_t *rb_night = radio_create("rb_night", 0, 0, "Midnight Navy",
                                    theme_engine_get_color(THEME_COLOR_BACKGROUND) == 0xFF002040);
    checkbutton_set_on_toggle(rb_night, scheme_radio_toggled, (void *)(uint64_t)0xFF002040);
    node_add_child(s_content_panel, rb_night);

    node_t *rb_desert = radio_create("rb_desert", 0, 0, "Desert Sand",
                                     theme_engine_get_color(THEME_COLOR_BACKGROUND) == 0xFF806000);
    checkbutton_set_on_toggle(rb_desert, scheme_radio_toggled, (void *)(uint64_t)0xFF806000);
    node_add_child(s_content_panel, rb_desert);
...
static void show_sound_settings(node_t *btn, void *userdata) {
    ...
    node_t *title = label_create("snd_title", 0, 0, "Sound", 0xFFE6E8EB);
    title->margin[2] = 8;
    node_add_child(s_content_panel, title);
    ...
    if (audio_available()) {
        node_t *hw = label_create("snd_hw", 0, 0,
            "AC'97 controller (QEMU Intel 82801AA) - Analog Line Out", 0xFFE6E8EB);
        hw->margin[2] = 12;
        node_add_child(s_content_panel, hw);
    } else {
        node_t *warn = label_create("snd_warn", 0, 0,
            "No AC'97 sound card. Start QEMU with: -device AC97,audiodev=aud0", 0xFFFF5F57);
        warn->margin[2] = 12;
    ...
    bar[25] = '\0';
    node_t *bar_lbl = label_create("snd_bar", 0, 0, bar, 0xFFE6E8EB);
    ...
    node_t *vol_lbl = label_create("snd_vol", 0, 0, vol_str, 0xFFE6E8EB);
    vol_lbl->margin[2] = 6;
    ...
    strcat(rate_str, " Hz");
    node_t *rate_lbl = label_create("snd_rate", 0, 0, rate_str, 0xFF99A1AF);
...
static void show_network_settings(node_t *btn, void *userdata) {
    ...
    node_t *title = label_create("net_title", 0, 0, "Network Settings", 0xFFE6E8EB);
    title->margin[2] = 20;
    node_add_child(s_content_panel, title);
    
    char ip_str[32] = "IP: Offline";
    
    node_add_child(s_content_panel, label_create("net_ip", 0, 0, ip_str, 0xFF99A1AF));
    node_add_child(s_content_panel, label_create("net_dhcp", 0, 0, "DHCP: Enabled", 0xFF99A1AF));
    node_add_child(s_content_panel, label_create("net_mac", 0, 0, "MAC: QEMU Default", 0xFF99A1AF));
...
static void settings_app_start(void) {
    ...
    // Sidebar
    node_t *sidebar = panel_create("settings_sidebar", 0, 0, 150, 400, 0xFF2D333D);
    ...
    // Content Panel
    s_content_panel = panel_create("settings_content", 0, 0, 450, 400, 0xFF252A33);
