...
static void explorer_start(void) {
    ...
    node_t *header = panel_create("explorer_header", 0, 0, 588, 42, 0xFF2D333D);
    header->layout_type = LAYOUT_HBOX;
    header->padding[0] = 8; header->padding[1] = 12;
    header->padding[2] = 8; header->padding[3] = 12;
    panel_set_border(header, 0xFF3D4450, 1);
    node_t *title = label_create("explorer_title", 0, 0, "FILES", 0xFFE6E8EB);
    title->flex_weight = 1;
    s_usage_label = label_create("explorer_usage", 0, 0, "SDFS", 0xFF99A1AF);
    node_add_child(header, title);
    node_add_child(header, s_usage_label);

    node_t *location = panel_create("explorer_location", 0, 0, 588, 30, 0xFF252A33);
    location->layout_type = LAYOUT_HBOX;
    location->padding[0] = 6; location->padding[1] = 10;
    location->padding[2] = 4; location->padding[3] = 10;
    panel_set_border(location, 0xFF4A5260, 1);
    s_path_label = label_create("explorer_path", 0, 0, "Location: /", 0xFFE6E8EB);
    ...
    panel_set_border(toolbar, 0xFF4A5260, 1);
    ...
    s_list_panel = panel_create("explorer_list", 0, 0, 588, 330, 0xFF1B1E24);
    ...
    panel_set_border(s_list_panel, 0xFF4A5260, 1);

    node_t *footer = panel_create("explorer_footer", 0, 0, 588, 34, 0xFF252A33);
    footer->layout_type = LAYOUT_HBOX;
    footer->padding[0] = 5; footer->padding[1] = 8;
    footer->padding[2] = 5; footer->padding[3] = 8;
    panel_set_border(footer, 0xFF4A5260, 1);
    s_status_label = label_create("explorer_status", 0, 0, "Ready", 0xFF99A1AF);
