...
static void focus_bus_handler(const gui_event_t *event, void *userdata) {
    ...
    /* The TAB key toggles the app launcher ONLY when no window is focused.
     * When a window is focused (e.g. the terminal), TAB is forwarded to it
     * so applications can use it for completion, focus cycling, etc. */
