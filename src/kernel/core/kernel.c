...
static void parse_multiboot2(uint32_t mbi_addr) {
  ...
  while (offset + 8 <= info->total_size) {
    ...
    } else if (tag->type == 14 || tag->type == 15) { /* ACPI RSDP (v1/v2) */
      /* O payload da tag é uma cópia do RSDP. */
      acpi_set_rsdp((uint64_t)mbi_addr + offset + 8);
...
void kernel_main(uint32_t magic, uint32_t mbi_addr) {
  ...
  const int safe_hardware_boot = 0;
...
init_mouse();
   ...
   acpi_init();
   init_apic(); /* Fase 3: LAPIC/IOAPIC + APs (SMP) */
   pci_init();
   boot_stage("pci ok");
   init_gpu();
   boot_stage("gpu ok");
  ...
  serial_print("[boot] NET INIT BLOCK REACHED\n");
  if (!safe_hardware_boot) {
    net_init();
    tcp_init();
    udp_init();
