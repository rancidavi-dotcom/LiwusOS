        ...
        if (record->type == 0) { // Processor Local APIC
            ...
            if (lapic->flags & 1) { // 1 = Enabled, or 2 = Online Capable
                if (cpu_count < 16) {
                    ...
                    serial_print_hex(lapic->acpi_processor_id);
...
void init_apic(void) {
    ...
    if (!acpi_available()) {
        serial_print("APIC: Error - ACPI unavailable!\n");
        return;
    }
    ...
    if (ioapic_phys_base != 0) {
        ...
        ioapic_route(irq_overrides[10], 43, 0); // Network (IRQ 10 with AC97) -> Vector 43
        ioapic_route(irq_overrides[11], 43, 0); // Network (IRQ 11 without AC97) -> Vector 43
