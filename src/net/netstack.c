...
static uint32_t netmask_ip = 0x00FFFFFF; // 255.255.255.0
...
/* ---- ARP neighbor cache (all ARP activity on the LAN) ---- */
#define ARP_CACHE_MAX 64
static netstack_neighbor_t arp_cache[ARP_CACHE_MAX];
static int arp_cache_count = 0;

static void arp_cache_record(uint32_t ip, const uint8_t mac[6]) {
  if (ip == 0 || ip == 0xFFFFFFFF) return;
  ...
  if (arp_cache_count < ARP_CACHE_MAX) {
...
void netstack_set_netmask(uint32_t mask) { netmask_ip = mask; }
uint32_t netstack_get_netmask(void)      { return netmask_ip; }
uint32_t netstack_get_gateway(void)      { return gateway_ip; }
void netstack_arp_cache_clear(void)      { arp_cache_count = 0; }
int  netstack_arp_cache_count(void)      { return arp_cache_count; }
int  netstack_arp_cache_get(int idx, uint32_t *ip, uint8_t mac[6]) {
  if (idx < 0 || idx >= arp_cache_count) return -1;
  *ip = arp_cache[idx].ip;
  memcpy(mac, arp_cache[idx].mac, 6);
  return 0;
}

void netstack_arp_probe(uint32_t dest_ip) {
  netstack_send_arp_request(dest_ip);
}
...
/* Debug switch: tests can flip this on for full serial visibility. */
volatile int g_net_verbose = 0;

static void net_log(const char *msg) {
    if (g_net_verbose) serial_print(msg);
}

static void net_log_u32(uint32_t value) {
    if (g_net_verbose) {
        char num[16];
...
static void net_log_ip(uint32_t ip) {
    if (g_net_verbose) {
        char num[16];
...
static void net_log_mac(const uint8_t mac[6]) {
    if (g_net_verbose) {
        for (int i = 0; i < 6; i++) {
            char h[8];
            itoa(mac[i] >> 4, h, 16); serial_print(h);
            itoa(mac[i] & 0xF, h, 16); serial_print(h);
            if (i < 5) serial_print(":");
        }
    }
}
...
void netstack_send_ipv4(uint32_t dest_ip, uint8_t proto, void *data,
  ...
  uint8_t dest_mac[6];
  if (len > 1480)
    return; // Fragment not supported (MTU 1500 - IPv4 header 20)
...
/* Sweep the local /24 subnet with ARP probes and collect responses.
 * Paces probes so the TX ring can drain. Blocks until collection is done. */
int netstack_scan_net(void) {
  extern volatile uint32_t timer_ticks;

  /* /24 subnet basis */
  uint32_t subnet = my_ip & 0xFFFFFF00;

  uint32_t self = my_ip;
  uint32_t start = timer_ticks;

  /* Fire ARP probes with gentle pacing (1 every ~2 ms) */
  for (uint32_t host = 1; host < 254; host++) {
    uint32_t ip = subnet | host;
    if (ip == self) continue;
    netstack_arp_probe(ip);

    /* Every 16 probes, wait ~20 ms so the TX ring can drain and we don't
       overwhelm the NIC.  Also allows ARP replies to arrive while we wait. */
    if ((host & 0x0F) == 0) {
      uint32_t s = timer_ticks;
      while ((timer_ticks - s) < 2) asm volatile("hlt");
    }
  }

  /* Collect for another ~1.2 s */
  while ((timer_ticks - start) < 150) asm volatile("hlt");
...
static int netstack_resolve_mac(uint32_t dest_ip, uint8_t out_mac[6]) {
  ...
  // Nunca bloquear com 'hlt' em contexto de IRQ (freeze garantido):
  // retorna sem cache e deixa o protocolo retransmitir.
  extern volatile uint8_t g_in_irq;
  if (g_in_irq) {
    net_log("[net] resolve mac skipped in irq ctx\n");
    return -1;
  }
...
extern void tcp_handle_packet(void *packet, uint16_t len, uint32_t src_ip,
                              uint32_t dest_ip);
extern void udp_handle_packet(void *packet, uint16_t len, uint32_t src_ip,
                              uint32_t dest_ip);
...
void netstack_handle_packet(void *data, uint16_t len) {
  ...
  serial_print("[NET_HDL] type=");
  {
      char num[16];
      itoa(type, num, 16); serial_print(num);
  }
  serial_print(" len=");
  {
      char num[16];
      itoa(len, num, 10); serial_print(num);
  }
  serial_print("\n");
  ...
  if (type == 0x0806) { // ARP
    ...
    arp_received = 1;
    arp_cache_record(arp->src_ip, arp->src_mac);
    ...
    serial_print("[NET_HDL] IPv4 packet\n");
    ipv4_packet_t *ip =
        (ipv4_packet_t *)((uintptr_t)data + sizeof(ethernet_frame_t));
    uint16_t ip_len = htons(ip->len);

    serial_print("[NET_HDL] IPv4 proto=");
    {
        char num[16];
        itoa(ip->proto, num, 10); serial_print(num);
    }
    serial_print("\n");

    // Aprende o vizinho que falou conosco (importante: o SYN que chega
    // ja aquece o cache do gateway, evitando bloquear com 'hlt' em IRQ)
    arp_cache_record(ip->src_ip, eth->src);
