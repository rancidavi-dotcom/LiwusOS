...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    const uint8_t *hdr;
    bs_t bs_frame[1];
    /* LiwusOS: scratch has ~13KB (grbuf[2][576] + syn[33][64] + ...).
     * Local on an 8KB task stack overflows -> make it static. The decoder
     * is single-threaded in this kernel (only the "media" task decodes). */
