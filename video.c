...
typedef struct { uint32_t magic, version, headersize, flags, numglyph, bytesperglyph, height, width; } PSF2_header;
typedef struct { uint16_t magic; uint8_t mode, charsize; } PSF1_header;

void set_clip(int x, int y, int w, int h) { 
    clip_x1 = x; clip_y1 = y; clip_x2 = x + w; clip_y2 = y + h; 
    if (clip_x1 < 0) clip_x1 = 0;
    if (clip_y1 < 0) clip_y1 = 0;
    if ((uint32_t)clip_x2 > target_width) clip_x2 = target_width;
    if ((uint32_t)clip_y2 > target_height) clip_y2 = target_height;
}

void reset_clip() { 
    clip_x1 = 0; clip_y1 = 0; 
    clip_x2 = target_width; clip_y2 = target_height; 
}

void video_set_target(uint32_t* buffer, uint32_t width, uint32_t height) {
    target_buffer = buffer; target_width = width; target_height = height;
    reset_clip();
}

void video_reset_target() {
    target_buffer = backbuffer; target_width = screen_width; target_height = screen_height;
    reset_clip();
}
...
void video_blit(uint32_t* source, int src_pitch, int sx, int sy, int w, int h, int dx, int dy) {
    for (int r = 0; r < h; r++) {
        int d_y = dy + r;
        if (d_y < clip_y1 || d_y >= clip_y2) continue;
        int s_y = sy + r;
        ...
        for (int c = start_c; c < end_c; c++) {
            ...
            if (p != 0) dest_row[c] = p; // Chroma key: skip black
        }
    }
}
...
void draw_filled_circle(int xm, int ym, int r, uint32_t color) {
    for (int y = -r; y <= r; y++) {
        for (int x = -r; x <= r; x++) {
            if (x * x + y * y <= r * r) draw_pixel(xm + x, ym + y, color);
        }
    }
}
...
void draw_mouse_cursor(int x, int y, int type) {
    ...
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 12; j++) {
            if (cursor[i][j] == 'X') draw_pixel(x + j, y + i, 0x000000);
            else if (cursor[i][j] == '.') draw_pixel(x + j, y + i, 0xFFFFFF);
        }
    }
}
...
void clear_screen(uint32_t color) {
    ...
    while(size >= 8) {
        ptr[0] = color; ptr[1] = color; ptr[2] = color; ptr[3] = color;
        ptr[4] = color; ptr[5] = color; ptr[6] = color; ptr[7] = color;
        ptr += 8; size -= 8;
    }
    while(size--) *ptr++ = color;
}
...
void draw_char(int x, int y, char c, uint32_t color) {
    ...
    if (font2->magic == 0x864ab572) {
        height = font2->height; width = font2->width; bpl = (width + 7) / 8;
        glyph = _binary_font_psf_start + font2->headersize + ((unsigned char)c * font2->bytesperglyph);
    } else {
        PSF1_header* font1 = (PSF1_header*)_binary_font_psf_start;
        height = font1->charsize; width = 8; bpl = 1;
        glyph = _binary_font_psf_start + sizeof(PSF1_header) + ((unsigned char)c * font1->charsize);
    }
    for (uint32_t cy = 0; cy < height; cy++) {
        for (uint32_t cx = 0; cx < width; cx++) {
            if (glyph[cy * bpl + cx / 8] & (0x80 >> (cx % 8))) draw_pixel(x + cx, y + cy, color);
        }
    }
}
...
void draw_string(int x, int y, const char* str, uint32_t color) {
    ...
    while (*str) {
        if (*str == '\n') { x = sx; y += 16; } 
        else { draw_char(x, y, *str, color); x += 8; }
        str++;
    }
}
...
rect_t video_rect_intersect(rect_t a, rect_t b) {
    ...
    if (x2 <= x1 || y2 <= y1) return (rect_t){0, 0, 0, 0};
